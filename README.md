...
可点「浏览…」选择）**或直接粘贴 B 站视频链接**——工具自动识别类型
（http(s) 开头按链接处理，BV/av/b23.tv、?p=N 分集均可，音轨自动下载转 wav）。
...
- **和弦进行实时提示**：顶部显示调性与当前和弦，进行序列随播放高亮推进，
  和弦标记同时叠加在乐谱对应位置上方（红色为当前和弦）；
